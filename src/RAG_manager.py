...
class RAGManager:
    ...
    def retrieve_documents(self, plant_groups: List[str], plant_description: str):
        retriever = self.create_retriever(plant_groups=plant_groups)
        retrieved_docs = retriever.invoke(plant_description)
        logger.info(f"Retrieved documents: {[doc.page_content for doc in retrieved_docs]}")
